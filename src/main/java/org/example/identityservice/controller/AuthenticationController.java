...
public class AuthenticationController {
    ...
    ApiResponse<AuthenticationResponse> authenticate(@RequestBody AuthenticationRequest request){
      boolean result = authenticationService.authenticate(request);
        ...
        return ApiResponse.<AuthenticationResponse>builder() // Lấy cái hộp TO ra
                .result( // Mở cái khe "result" của hộp to ra, bắt đầu nhét đồ vào:
                        AuthenticationResponse.builder() // Lấy cái hộp NHỎ ra
                                .authenticated(result)    // Bỏ kết quả vào hộp nhỏ
                                .build()                  // Đóng nắp hộp NHỎ
                ) // Hết quá trình nhét đồ vào hộp to
                .build(); // Đóng nắp hộp TO lại, và gửi về (return)
    }
